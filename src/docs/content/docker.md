...
containerd 默认路径位于 `/opt/containerd`，修改默认路径的方法如下

1. 输入 `containerd config default > /etc/containerd/config.toml` 创建默认配置文件
2. 找到 `[plugins."io.containerd.internal.v1.opt"]`
3. 修改该项下的 `path = "/opt/containerd"`
4. 输入 `systemctl restart containerd.service`
