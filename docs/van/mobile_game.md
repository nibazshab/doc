---
sidebarDepth: 0
---

# 大杂烩

<br>

## 实名认证

::: details 仅供测试或交流学习使用，内容来自网络
张民 110105197001137135
:::

## 王者荣耀 q 币充点券

仅限安卓 qq 区游戏号可用（ 微信区没有测试过，请自行测试是否可用 ）

> https://pay.qq.com/h5/index.shtml?m=buy&c=game&dialog=0&midasApiVersion=5&transactionid=CC8E5656-EEC9-4ADD-865E-D4D81C258D691558348901265&dh=1&pf=mds_storeopen_qb-_mds_default_v1_0_0.navgame_v1.0-android&appid=1450002258&zoneid=(区号+1010)&n=(点卷数量)&as=1&aid=&wxAppid2=wx951bdcac522929b6&u=(游戏qq号)

* (区号+1010) - 填写游戏账号所在的大区号 + 1010 所得到的数字，例如游戏账号在 153 区，则填写 1163
* (点卷数量) - 填写要充值的点卷数量
* (游戏qq号) - 填写游戏账号拥有人的 qq 号，例如游戏账号是用张三的 qq 号 666000 注册的，则填写 666000

::: details 示例（ 注：示例账号与本人无关 ）
* 大区：手q 153 区
* 点券：10
* 游戏qq号：666000
```
https://pay.qq.com/h5/index.shtml?m=buy&c=game&dialog=0&midasApiVersion=5&transactionid=CC8E5656-EEC9-4ADD-865E-D4D81C258D691558348901265&dh=1&pf=mds_storeopen_qb-_mds_default_v1_0_0.navgame_v1.0-android&appid=1450002258&zoneid=1163&n=10&as=1&aid=&wxAppid2=wx951bdcac522929b6&u=666000
```
:::

填完相关信息后，把括号删除，充值人在 qq 内点击该链接，即可使用 q 币为被充值人的游戏账号充值点券
