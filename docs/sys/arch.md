...
官方的 Arch 安装程序是基于 CLI 命令行界面,对于那些需要 GUI 图形界面安装环境的用户，可以选择这些 Live CD:

* SteamOS - SteamOS 3.0 基于 Arch Linux 所开发，为电子游戏而设计
* Manjaro Linux - 一个提供 XFCE/KDE/GNOME 桌面环境的发行版
* Arco Linux - 一个打包并配置好了各种 DE 以及 WM 环境的发行版
* EndeavourOS - 一个包含了多种桌面环境的图形化安装器的发行版
* ArchBang - 一个默认使用 OpenBox 桌面环境的发行版
...
* [维基百科](https://wiki.archlinux.org/title/Arch_Linux)
