...
承接 [基础篇](ArchInstall.md) 的内容，本篇将介绍如何安装图形化桌面环境
...
如果已添加 archlinuxcn 软件源，输入 `pacman -S yay` 直接安装 AUR 助手 yay

如果未添加 archlinuxcn 软件源，如下输入指令编译安装 AUR 助手 yay
...
输入 `pacman -S gnome` 安装 GNOME 桌面环境，输入 `systemctl enable gdm.service` 启用 GNOME 桌面环境

在以后的图形化使用中，请以用户 atri 登录系统
